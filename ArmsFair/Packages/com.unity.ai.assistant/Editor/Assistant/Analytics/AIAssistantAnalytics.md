...
Sent via relay bus POST (subject to a 5-minute debounce). A final flush is always sent when the session ends. The payload wraps an array of events under the `ai_gateway_events` key.
...
| `messages`         | array   | Full conversation history up to and including this turn. Each element is a message object with `role`, `isComplete`, `timestamp`, and `blocks` fields (see `AssistantMessage.ToJson()`). |
